...
fun Activity.openEditorIntent(path: String, forceChooser: Boolean, applicationId: String) {
    Thread {
        val newUri = Uri.fromFile(File(path))//getFinalUriFromPath(path, applicationId) ?: return@Thread
        Intent().apply {
...
fun Activity.openPathIntent(path: String, forceChooser: Boolean, applicationId: String, forceMimeType: String = "") {
    Thread {
        ...
        Intent().apply {
            ...
            if (applicationId == "gallery.quickpic.photos.album.photoeditor.fotos") {

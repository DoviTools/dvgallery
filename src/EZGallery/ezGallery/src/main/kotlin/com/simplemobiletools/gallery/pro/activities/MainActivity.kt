...
import com.simplemobiletools.gallery.pro.dialogs.InitializingDialogFragment
...
import com.thcommon.ui.RateStartsActivity
...
class MainActivity : SimpleActivity(), DirectoryOperationsListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState != null) {
            ThConfigHost.setLaunchCount(this, ThConfigHost.getLaunchCount(this) + 1)
            showRateIfNeeded()
        }
    ...
    private fun showRateIfNeeded() {
        if (!ThConfigHost.getRateNeverShow(this)) {
            val launchCount = ThConfigHost.getLaunchCount(this)
            if (launchCount == 2 || launchCount == 6 || launchCount == 15) {
                RateStartsActivity.show(this, getString(R.string.app_name), getString(R.string.app_internal_name), "mobcoolgames@gmail.com")
            }
        }
    }
    ...
    private fun getDirectories() {
        ...
        Log.d("MainActivity", "Begin GetDirectory")

        getCachedDirectories(getVideosOnly, getImagesOnly, mDirectoryDao) {
            Log.d("MainActivity", "After getCachedDirectories")
    ...
    private fun showAllMedia() {
        config.showAll = true
        config.groupBy = GROUP_BY_DATE_TAKEN or GROUP_DESCENDING
    ...
    private fun gotDirectories(newDirs: ArrayList<Directory>) {
        ...
        runOnUiThread {
            showFirstOpenLoadingDialog()
        }
        val foldersToScan = mediaFetcher.getFoldersToScan()
        runOnUiThread {
            dismissFirstOpenLoadingDialog()
        }
    ...
    private fun showFirstOpenLoadingDialog() {
        if (baseConfig.hasFirstLoaded) {
            return
        }
        val dialog = supportFragmentManager.findFragmentByTag("InitializingDialogFragment")
        if (dialog != null && dialog.isAdded) {
            return
        }

        InitializingDialogFragment.newInstance().show(supportFragmentManager, "InitializingDialogFragment")
        baseConfig.hasFirstLoaded = true
    }

    private fun dismissFirstOpenLoadingDialog() {
        val dialog = supportFragmentManager.findFragmentByTag("InitializingDialogFragment")
                ?: return
        val dialogFragment = dialog as DialogFragment
